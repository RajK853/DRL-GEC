...
def get_best_candidates(mask_gen, tokens, reference, tok_i, tok_edit, src_ref_lev_dist, verbose=False):
    ...
    delta = src_ref_lev_dist - distances
    candidate_mask = delta > 0                          # Reduction in lev distance
    candidate_labels = tok_labels[candidate_mask]
    candidate_delta = delta[candidate_mask]
    if verbose:
...
def get_best_label(policy, mask_gen, tokens, tok_i, candidate_labels, candidate_delta):
...
def search_best_actions(policy, tokens, reference, mask_gen, verbose=False):
    # Search labels
    ...
    if tokens != reference:
        ...
        for tok_i, tok_edit in reversed(tuple(enumerate(edit_mask))):
            ...
            tok_label = get_best_label(policy, mask_gen, tokens, tok_i, candidate_labels, candidate_delta)
            if tok_label != "$KEEP":
